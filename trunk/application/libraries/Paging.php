...
class paging{
   ...
   function __construct() {
    ...
    function create_pagination($pg_config) { 
        ...
        $config['first_link']      = 'Awal';       //The text you would like shown in the "first" link on the left. If you do not want this link rendered, you can set its value to FALSE.
        $config['first_tag_open']   = '<li>';      //The opening tag for the "first" link.
        $config['first_tag_close']  = '</li>';     //The closing tag for the "first" link.

        $config['last_link']       = 'Akhir';        //The text you would like shown in the "last" link on the right. If you do not want this link rendered, you can set its value to FALSE.
        $config['last_tag_open']   = '<li>';      //The opening tag for the "last" link.
        $config['last_tag_close']  = '</li>';     //The closing tag for the "last" link.
        
        $config['next_link']       = '&raquo;';     //The text you would like shown in the "next" page link. If you do not want this link rendered, you can set its value to FALSE.
        $config['next_tag_open']   = '<li>';      //The opening tag for the "next" link.
        $config['next_tag_close']  = '</li>';     //The closing tag for the "next" link.
        
        $config['prev_link']       = '&laquo;';     //The text you would like shown in the "previous" page link. If you do not want this link rendered, you can set its value to FALSE.
        ...
        $config['cur_tag_open']    = '<li class="active"><a>';         //$config['cur_tag_open'] = '<b>';
        ...
        // $index_segment  = $this->CI->uri->segment(2);
        ...
        }elseif(is_numeric($last_segment)){ //end with number , 2
            $config["base_url"] = $this->get_current_url_without_pagenum();
        }elseif($last_segment != 'index'){//jika akhir uri tidak pakai "index"
            $config["base_url"] = $this->get_current_url()."/index";
        ...
        $data["ordinal_num"]   = 1+(($todb['page_no']-1)*$todb['limit'] > 0? ($todb['page_no']-1)* $todb['limit'] : 0 );
        return $data;
    }
